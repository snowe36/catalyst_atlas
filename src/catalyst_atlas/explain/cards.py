...
def format_cryptic_hero(
    ...
    seq_identity_note: str = "synthetic demo (cluster-lookup placeholders, not real BLAST %id)",
) -> str:
    ...
    lines = [
        ...
        f"**Sequence context:** {seq_identity_note}",
        "",
        "| Method | Inferred chemistry |",
        "|---|---|",
        f"| Seq cluster-lookup (BLAST placeholder) | `{seq_baseline_chem}` |",
        f"| Fold cluster-lookup (Foldseek placeholder) | `{fold_baseline_chem}` |",
